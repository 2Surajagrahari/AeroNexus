...
function AnimatedFlightPath({ originCoords, destCoords, showWeather }) {
    const map = useMap();
    const [planeIndex, setPlaneIndex] = useState(0);
    const [dynamicPositions, setDynamicPositions] = useState([]);
    const [isComputing, setIsComputing] = useState(false);

    // Compute Math Path Asynchronously due to OpenWeatherMap API wait times
    useEffect(() => {
        let active = true;
        setPlaneIndex(0);
        setIsComputing(true);
        ...
        const computeRoute = async () => {
            // Live avoidance probe if weather mode enabled, else just compute generic math
            const newPath = await calculateSafeRoute(originCoords, destCoords);
            if (active) {
                setDynamicPositions(newPath);
